...
```
```{r}
forecast_ts<-modified_ts
ma_impute <- function(data) {
  imputed_data <- na_ma(data)
...
arima_ts_test <- arima_ts[80:100,]
arima_ts_train <- arima_ts[1:80,]
```

```{r}
drop_observations_seasonally <- function(ts, fraction, seasonality) {
  n <- length(ts)
  period <- length(time(ts)) / seasonality
  
  # Determine indices to drop seasonally
  indices_to_drop <- rep(1:seasonality, each = round(fraction * period))
  indices_to_drop <- sort(rep(indices_to_drop, length.out = n))
  
  ts[indices_to_drop] <- NA
  return(ts)
}
...
seasonally_modified_ts<-drop_observations(arima_ts_train, fraction_missing,'seasonal',4)
chunked_modified_ts<-drop_observations(arima_ts_train, fraction_missing,'chunked')
```


```{r}
# 3. Impute Missing Data# 3. Impute Missing Data
# Imputing by weighted moving average, LOCF, NOCB
ma_ts<- ma_impute(modified_ts)

locf_ts <- modified_ts %>%
  mutate(value_locf = ifelse(is.na(value),
  na_locf(value), value))
...
ma_seas_ts <- ma_impute(seasonally_modified_ts)
