...
```{r}
generate_multiple<- function(impute_func){
  n <- 100  # Number of observations
  order <- c(1, 1, 1)  # ARIMA order (p, d, q)
  ar_param <- 0.5  # AR parameter
  ma_param <- 0.7  # MA parameter
  ...
  for (i in (1:reps)){
    ...
    seasonally_modified_ts<-drop_observations(arima_ts_train, fraction_missing/12,'seasonal',4)
    chunked_modified_ts<-drop_observations(arima_ts_train, fraction_missing,'chunked')
    ma_chunked_ts <- na_ma(chunked_modified_ts)
    estimated_params <- estimate_arima_params(modified_ts$value)
    modified_ts<-impute_func(modified_ts)
    datasets[i] <- list(arima_ts, modified_ts) 
  }
  return(datasets)
}
```
```{r}
datasets<-generate_multiple(na_ma)
forecast_results <- arima_forecast_framework(datasets, order, ar_param, ma_param, forecast_horizon)
...
```{r}
df_generator_helper <- function(option){
  n <- 100
  ar_param <- 0.5
  ma_param <- 0.7
  sd_noise <- 1
  ...
  chunked_modified_ts<-drop_observations(arima_ts_train, fraction_missing,'chunked')
  adjusted_ts <- modified_ts %>%
    mutate(value = ifelse(is.na(value),
    na_locf(value, option = option), value))
  
  adjusted_seas_ts <- seasonally_modified_ts %>%
    mutate(value = ifelse(is.na(value),
    na_locf(value, option = option), value))
  
  adjusted_chunked_ts <- chunked_modified_ts %>%
    mutate(value = ifelse(is.na(value),
    na_locf(value, option = option), value))
...
```{r}
#
locf_results <- df_generator_helper("locf")
nocb_results <- df_generator_helper("nocb")
```
