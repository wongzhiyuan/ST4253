...
compute_forecast_horizon <- function(ts_data, order, ar, ma, h) {
  model <- Arima(ts_data$value, order = order,method='ML' )
...
df_generator_helper <- function(impute_function){
  n <- 100
  ar_param <- 0.5
  ma_param <- 0.7
  sd_noise <- 1
...
repeater <- function(num , impute){
  result = list()
  for (i in 1:num){
    res <- df_generator_helper(impute)
    result[[i]] <- res
  }
  return(result)
}
...
```{r}
locf_results <- df_generator_helper(impute_locf)
nocb_results <- df_generator_helper(impute_nocb)
linear_results <- df_generator_helper(impute_linear)
```
```{r}
ma_results<-df_generator_helper(na_ma)
kalman_results<-df_generator_helper(na_kalman)
```
```{r}
all_methods <-list(list('impute_locf',impute_locf),list('impute_nocb',impute_nocb),list('impute_linear',impute_linear))
df_impute_all(all_methods)
```
```{r}
...
```{r}
all_forecast_data<-forecast_generate(all_methods,20)
```
